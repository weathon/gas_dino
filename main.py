...
parser.add_argument("--fusion", type=str, default="concat", choices=["concat", "cross_attention", "early"])
...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, mode="train"):
        ...
        if mode == "train":
            ...
            self.images = random.sample(self.images, 64)
        ...
        if args.use_bsvunet2_style_dataaug:
            self.sperate_aug = torchvision.transforms.v2.Compose([
                torchvision.transforms.v2.RandomResizedCrop(args.image_size, scale=(0.98, 1.02)),
    ...
    def __getitem__(self, idx):  
        ...
        if self.mode == "train":  
            ...
            X *= 1 + torch.randn(9)[:,None,None] * 0.1
        else:
            X = X[:-1]/255.0
            Y = Y/255.0 
            ROI = ROI > 0
        ...
        if i%args.print_every==0:
            with torch.no_grad():
                ...
                for b in range(1): 
                    ...
                    write_log(f"Epoch {epoch} Val Loss {round(total_loss, 3)} Val IOU {round(total_iou, 3)}")
                    write_log(f"Epoch {epoch} Train Loss {round(np.mean(train_running_loss), 3)} Train IOU {round(np.mean(train_running_iou), 3)}")
                    train_running_loss = []
                    train_running_iou = []
