...
class BCA_lite(nn.Module):
    ...
    def __init__(self, dim=384):
        ...
        self.decoder = nn.TransformerEncoderLayer(d_model=384 * 2, nhead=8, dim_feedforward=args.ffn_dim, dropout=args.dropout, batch_first=True),
...
class MyModel(nn.Module):
    def __init__(self, backbone):
        ...
        self.bca = nn.ModuleList([BCA_lite() for _ in range(args.num_decoder_layers)])  
    ...
    def forward(self, current_frame, long_bg, short_bg):
        if not args.use_pre_extracted_features:
        ...
        for bca in self.bca:
            current_frame = bca(current_frame, long_bg, short_bg)
...
if not args.use_pre_extracted_features:
    ...
    train_dataloader = torch.utils.data.DataLoader(PreExtractedDataset("train"), batch_size=args.batch_size, shuffle=True, num_workers=cpu_counts, persistent_workers=True, prefetch_factor=3)
    val_dataloader = torch.utils.data.DataLoader(PreExtractedDataset("val"), batch_size=args.batch_size, shuffle=True, num_workers=cpu_counts, persistent_workers=True, prefetch_factor=3)
            ...
            with torch.no_grad():
                ...
                for X_val, Y_val, ROI_val in val_dataloader:
                    ...
                    if not args.use_pre_extracted_features:
                        ...
                        pred = mymodel(X_val[:,0], X_val[:,1], X_val[:,2]) * (ROI_val.cuda().float().unsqueeze(1) > 0)
